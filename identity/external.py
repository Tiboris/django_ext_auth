...
class RemoteUserAttrMiddleware(RemoteUserMiddleware):
    ...
    def update_user_groups(self, request):
        ...
        ext_groups_singlestring = request.META.get(
            self.header + "_GROUPS", None)
    ...
    def process_request(self, request):
        ...
        if hasattr(request, 'user') and request.user.is_authenticated() and \
            ...
            request.user.get_username() == self.clean_username(
            ...
            if isinstance(stored_backend, RemoteUserBackend):
                ...
                if lastname is not None:
                    ...
                    need_save = True
                if need_save or \
                        request.META.get(self.header + "_GROUP_N", None) or \
                        request.META.get(self.header + "_GROUPS", None):
